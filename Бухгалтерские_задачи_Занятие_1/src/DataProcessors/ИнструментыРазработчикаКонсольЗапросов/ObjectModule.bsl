#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	// Поместить запросы во временное хранилище.
	// 
	// Параметры:
	//  Объект Объект
	//  ИдентификаторТекущегоЗапроса Идентификатор текущего запроса
	//  ИдентификаторТекущегоПараметра Идентификатор текущего параметра
	// 
	// Возвращаемое значение:
	//  Строка - Поместить запросы во временное хранилище
	...
	Функция ПоместитьНастройкиВоВременноеХранилище(Объект) Экспорт
		...
		//Сизов
		ПараметрыПередачи.Вставить("ЦветШрифт", Объект.ЦветШрифт);
		ПараметрыПередачи.Вставить("ЦветЗапрос", Объект.ЦветЗапрос);
		ПараметрыПередачи.Вставить("ШрифтЗапрос", Объект.ШрифтЗапрос);
	...
	Функция ЗаписатьЗапросыВФайлXML(знач Объект) Экспорт
		...
		// Цикл запросов.
		//Сизов +
		ФайлXML.ЗаписатьАтрибут("Шрифт",XMLСтрока(ЗначениеВСтрокуВнутр(ШрифтЗапрос)));
		ФайлXML.ЗаписатьАтрибут("ЦветФон",XMLСтрока(ЗначениеВСтрокуВнутр(ЦветЗапрос)));
		ФайлXML.ЗаписатьАтрибут("ЦветШрифт",XMLСтрока(ЗначениеВСтрокуВнутр(ЦветШрифт)));
		...
		//
		//ФайлXML.ЗаписатьНачалоЭлемента("query");
		//ФайлXML.ЗаписатьАтрибут("name", XMLСтрока("%%ЦветШрифт%%"));
		//ФайлXML.ЗаписатьНачалоЭлемента("text");
		//ФайлXML.ЗаписатьТекст( XMLСтрока(ЗначениеВСтрокуВнутр(ЦветШрифт)));
		//ФайлXML.ЗаписатьКонецЭлемента();
		//ФайлXML.ЗаписатьНачалоЭлемента("parameters");
		//ФайлXML.ЗаписатьКонецЭлемента();
		//ФайлXML.ЗаписатьКонецЭлемента();
	...
	Функция ПрочитатьЗапросыИзФайлаXML(ДвоичныеДанные) Экспорт
		...
		Пока ФайлXML.ПрочитатьАтрибут() Цикл
			...
			Если ФайлXML.Имя = "Шрифт" Тогда
			
				ШрифтЗапрос = ЗначениеИзСтрокиВнутр(ФайлXML.Значение);
			
			КонецЕсли;
			Если ФайлXML.Имя = "ЦветФон" Тогда
			
				ЦветЗапрос = ЗначениеИзСтрокиВнутр(ФайлXML.Значение);
			
			КонецЕсли;
			Если ФайлXML.Имя = "ЦветШрифт" Тогда
			
				ЦветШрифт = ЗначениеИзСтрокиВнутр(ФайлXML.Значение);
				...
				Если ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "query" Тогда 
					Пока ФайлXML.ПрочитатьАтрибут() Цикл 
						Если ФайлXML.Имя = "name" Тогда
							...
							//КонецЕсли;
							//
							//Если ФайлXML.Значение = "%%ЦветШрифт%%" Тогда
							//	ЭтоЦветШрифт = Истина;
							//	Прервать;
							//Иначе
							//	ЭтоЦветШрифт = Ложь;
